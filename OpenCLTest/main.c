...
static int validate(cl_float* input, cl_float* output) {
    ...
    fprintf(stdout, "validated %i values\n", i);
    return 1;
}

void cpu_square(cl_float* input, cl_float* output) {
    int i;
    for (i = 0; i < NUM_VALUES; i++) {
...
int main (int argc, const char * argv[]) {
    int i;
    char name[128];
    struct timeval tv_start1, tv_start2, tv_gpu, tv_cpu;
    ...
    if (gpu_queue == NULL) {
        gpu_queue = gcl_create_dispatch_queue(CL_DEVICE_TYPE_CPU, NULL);
    }

    cl_device_id gpu = gcl_get_device_id_with_dispatch_queue(gpu_queue);
    clGetDeviceInfo(gpu, CL_DEVICE_NAME, 128, name, NULL);
    fprintf(stdout, "Created a dispatch queue using the %s\n", name);

    
    dispatch_queue_t cpu_queue = gcl_create_dispatch_queue(CL_DEVICE_TYPE_CPU, NULL);

    cl_device_id cpu = gcl_get_device_id_with_dispatch_queue(cpu_queue);
    ...
    float* test_out_gpu = (float*)malloc(sizeof(cl_float) * NUM_VALUES);
    float* test_out_cpu = (float*)malloc(sizeof(cl_float) * NUM_VALUES);
    ...
    dispatch_sync(gpu_queue, ^{
        ...
        fprintf(stdout, "GPU workgroup size: %li\n", wgs);
        ...
        gcl_memcpy(test_out_gpu, mem_out, sizeof(cl_float) * NUM_VALUES);
    });
    gettimeofday(&tv_gpu, NULL);
    
    startms = tv_start1.tv_sec * 1000LL + tv_start1.tv_usec / 1000;
    endms = tv_gpu.tv_sec * 1000LL + tv_gpu.tv_usec / 1000;
    fprintf(stdout, "GPU msecs: %lli\n", endms - startms);
    
    gettimeofday(&tv_start2, NULL);

    dispatch_sync(cpu_queue, ^{
        // Although we could pass NULL as the workgroup size, which would tell
        // OpenCL to pick the one it thinks is best, we can also ask
        // OpenCL for the suggested size, and pass it ourselves.
        size_t wgs;
        ...
        fprintf(stdout, "CPU workgroup size: %li\n", wgs);
        // The N-Dimensional Range over which we'd like to execute our
        // kernel.  In this case, we're operating on a 1D buffer, so
        // it makes sense that the range is 1D.
        cl_ndrange range = {                                              // 6
        ...
        gcl_memcpy(test_out_cpu, mem_out, sizeof(cl_float) * NUM_VALUES);
    });

    // cpu_square(test_in, test_out_cpu);

    gettimeofday(&tv_cpu, NULL);
    
    startms = tv_start2.tv_sec * 1000LL + tv_start2.tv_usec / 1000;
    endms = tv_cpu.tv_sec * 1000LL + tv_cpu.tv_usec / 1000;
    fprintf(stdout, "CPU msecs: %lli\n", endms - startms);
    
    // Check to see if the kernel did what it was supposed to:
    if ( validate(test_in, test_out_gpu)) {
        fprintf(stdout, "All GPU values were properly squared.\n");
    }
    if ( validate(test_in, test_out_cpu)) {
        fprintf(stdout, "All CPU values were properly squared.\n");
    }
